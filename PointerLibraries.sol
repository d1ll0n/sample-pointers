...
library MemoryPointerLib {
  function copy(MemoryPointer src, MemoryPointer dst, uint256 size) internal view {
    assembly {
      ...
      if or(lt(returndatasize(), size), iszero(success)) {
        revert(0, 0)
      }
    }
  }
...
library MemoryWriters {
  ...
  function addDirtyBitsBefore(MemoryPointer mPtr, uint256 offset) internal pure {
    uint256 dirtyBits = type(uint256).max << (256 - offset);
    mPtr.write(mPtr.readUint256() | dirtyBits);
  }
  ...
  function addDirtyBitsAfter(MemoryPointer mPtr, uint256 offset) internal pure {
    uint256 dirtyBits = type(uint256).max >> offset;
    mPtr.write(mPtr.readUint256() | dirtyBits);
  }
